...
public final class HUDUtils
{
	 ...
	 * bound using Minecraft.getMinecraft().getTextureManager().bindTexture(
	 * resourceLocation).
	 * 
	 ...
	 * image
	 * @param textureHeight the height of the box texture in the resource
	 * location image
	 ...
	 * The provided ResourceLocation object will be bound using
	...
	public static void drawTexturedModalRect (int x, int y, int u, int v, int width, int height, float zLevel)
	{
		...
		buf.pos(x + 0, y + height, zLevel).tex( (u + 0) * var7, (v + height) * var8).endVertex();
		buf.pos(x + width, y + height, zLevel).tex( (u + width) * var7, (v + height) * var8).endVertex();
		buf.pos(x + width, y + 0, zLevel).tex( (u + width) * var7, (v + 0) * var8).endVertex();
		buf.pos(x + 0, y + 0, zLevel).tex( (u + 0) * var7, (v + 0) * var8).endVertex();
		t.draw();
	}
	...
	public static void renderItemOverlayIntoGUI (FontRenderer fontRenderer, ItemStack itemStack, int x, int y, boolean showDamageBar, boolean showCount)
	{
		if ( (itemStack != null) && (showDamageBar || showCount))
		{
			...
			if (showCount)
			{
				...
				if (itemStack.getMaxStackSize() > 1) count = HUDUtils.countInInventory(Minecraft.getMinecraft().player, itemStack.getItem(), itemStack.getItemDamage());
				else if (itemStack.getItem().equals(Items.BOW)) count = HUDUtils.countInInventory(Minecraft.getMinecraft().player, Items.ARROW);
	 ...
	 */
	public static void renderQuad (Tessellator t, int x, int y, int width, int height, int color)
	{
		VertexBuffer buf = t.getBuffer();
		buf.begin(7, DefaultVertexFormats.POSITION_TEX);
		buf.pos(x + 0, y + 0, 0.0D).putColor4(color);
		buf.pos(x + 0, y + height, 0.0D).putColor4(color);
		buf.pos(x + width, y + height, 0.0D).putColor4(color);
		buf.pos(x + width, y + 0, 0.0D).putColor4(color);
		t.draw();
	}

	public static int countInInventory (EntityPlayer p, Item i)
	{
		return countInInventory(p, i, -1);
	}

	public static int countInInventory (EntityPlayer p, Item i, int md)
	{
		int count = 0;
		for (int a = 0; a < p.inventory.mainInventory.length; a++)
			if ( (p.inventory.mainInventory[a] != null) && i.equals(p.inventory.mainInventory[a].getItem()) && ( (md == -1) || (p.inventory.mainInventory[a].getMetadata() == md))) count += p.inventory.mainInventory[a].stackSize;
		return count;
	}
