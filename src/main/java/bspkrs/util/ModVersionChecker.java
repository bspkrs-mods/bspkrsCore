 ...
 * compares it against the locally provided version string. It uses a natural
 * order comparator to determine if the remote version is newer than our local
 * version.
 * 
 * @author bspkrs
 */
...
public class ModVersionChecker
{
	...
	public ModVersionChecker (String modID, String curVer, String versionURL, String updateURL, String[] loadMsg, String[] inGameMsg, int timeoutMS)
	{
		...
		try
		{
			if (versionURL.startsWith("http://dl.dropboxusercontent.com")) versionURL = versionURL.replaceFirst("http", "https");
		...
		if (versionLines.length == 0 || versionLines[0].trim().equals("<html>")) newVersion = CHECK_ERROR;
		else newVersion = versionLines[0].trim();
		...
		if (versionCheckTracker == null) versionCheckTracker = Preferences.userNodeForPackage(this.getClass()).node("modversiontracker" + Const.MCVERSION);
		...
		if (! (new StringBuilder("@").append("MOD_VERSION@")).toString().equals(currentVersion) && !"${mod_version}".equals(currentVersion))
		{
			...
			if (lastNewVersionFound.equals("<html>")) lastNewVersionFound = currentVersion;

			runsSinceLastMessage = versionCheckTracker.node("runs_since_last_message").getInt(modID, 0);

			if (errorDetected) newVersion = lastNewVersionFound;
			...
			else runsSinceLastMessage = runsSinceLastMessage % 10;
		...
		if (versionLines.length > 1 && versionLines[1].trim().length() != 0) this.updateURL = versionLines[1];
	...
	{
		if (!isCurrentVersion(currentVersion, newVersion)) for (String msg : loadMsg)
			BSLog.info(msg);
	}
	...
	public static String[] checkVersionForMod (String modID)
	{
		...
		if (versionCheckerMap.containsKey(modID.toLowerCase(Locale.US)))
		{
			...
			{
				if (!isCurrentVersion(versionChecker.currentVersion, versionChecker.newVersion)) r = versionChecker.getInGameMessage();
				else r = new String[]{ I18n.format("bspkrs.modversionchecker.uptodate", versionChecker.modID) };
			}
			else r = new String[]{ I18n.format("bspkrs.modversionchecker.error", versionChecker.modID) };
		}
		else r = new String[]{ I18n.format("bspkrs.modversionchecker.invalidmodid") };
