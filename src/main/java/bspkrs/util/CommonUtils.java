...
public final class CommonUtils
{
	...
	{
		for (int i = 0; i < s.length(); i++)
			if (!Character.isLetterOrDigit(s.charAt(i)) && !chrs.contains(s.subSequence(i, i + 1))) return false;
		return true;
	}
		...
		if (list.trim().length() != 0)
		{
			...
			for (String item : items)
			{
				...
				if (parseInt(values[0]) == id)
				{
					...
					else if ( (values.length == 2) && (parseInt(values[1]) == md)) return true;
				}
			}
		}
		return false;
	...
	public static boolean isIDInList (String id, int md, String list)
	{
		...
		for (int i = 0; i < itemArray.length; i++)
		{
			...
			if ( (values.length > 1) && values[0].trim().equals(id) && (parseInt(values[1], -1) == md)) return true;
			else if (values[0].trim().equals(id)) return true;
		}
		return false;
	}
	...
	public static boolean isIDInList (int id, int md, String list)
	{
		...
		for (int i = 0; i < itemArray.length; i++)
		{
			...
			if ( (values.length > 1) && (tempID == id) && (parseInt(values[1], -1) == md)) return true;
	...
	public static int[] stringToBlock (String string)
	{
		...
		if (src.length < 1) return values;
		values[0] = parseInt(src[0]);
		if (src.length < 2) return values;
	...
	public static boolean isBlockInGroups (int[] block, int[][][] groups)
	{
		...
		{
			if (indexOfBlock(block, group) > -1) return true;
		}
		return false;
	}
	...
	public void playAtPitch (int i, World world, EntityPlayer p)
	{
		float f = (float) Math.pow(2D, (i - 12) / 12D);
		world.playSound(p.posX, p.posY, p.posZ, SoundEvent.REGISTRY.getObject(new ResourceLocation("note.pling")), SoundCategory.MUSIC, 0.5F, f, false);
	}
	...
	{
		while ( (pos.getY() > 0) && (world.isAirBlock(pos) || !world.isBlockNormalCube(pos, true) || world.getBlockState(pos).getBlock().isWood(world, pos)))
		{
			pos = pos.down();
		}
		return pos.getY();
	...
	{
		return (int) Math.round(Math.sqrt(CommonUtils.sqr(endPos.getX() - startPos.getX()) + CommonUtils.sqr(endPos.getZ() - startPos.getZ()) + CommonUtils.sqr(endPos.getY() - startPos.getY())));
	}
	...
	public static void setHugeMushroom (World world, Random random, BlockPos pos, IBlockState state)
	{
		...
		byte cw = 4;
		byte h = 4;
		fillWithBlocks(world, new BlockPos( (x - w) + 1, y, z - w), new BlockPos( (x + w) - 1, (y + h) - 1, z - w), state.getBlock().getDefaultState());
		fillWithBlocks(world, new BlockPos( (x - w) + 1, y, z + w), new BlockPos( (x + w) - 1, (y + h) - 1, z + w), state.getBlock().getDefaultState());
		fillWithBlocks(world, new BlockPos(x - w, y, (z - w) + 1), new BlockPos(x - w, (y + h) - 1, (z + w) - 1), state.getBlock().getDefaultState());
		fillWithBlocks(world, new BlockPos(x + w, y, (z - w) + 1), new BlockPos(x + w, (y + h) - 1, (z + w) - 1), state.getBlock().getDefaultState());
		fillWithBlocksRounded(world, new BlockPos(x - cw, y + h, z - cw), new BlockPos(x + cw, y + h, z + cw), state.getBlock().getDefaultState());
	}
		...
		for (int x = pos1.getX(); x <= pos2.getX(); x++)
			for (int y = pos1.getY(); y <= pos2.getY(); y++)
				for (int z = pos1.getZ(); z <= pos2.getZ(); z++)
				{
					...
					if ( ( (xd * xd) + (yd * yd) + (zd * zd)) <= 1.5D) world.setBlockState(new BlockPos(x, y, z), state, 3);
				}
	}
	...
	public static String getMCTimeString (long worldTime, long fmt)
	{
		...
		if (fmt == 24L) hour = (HH < 10 ? "0" : "") + String.valueOf(HH);
		else hour = String.valueOf(HH == 0 ? 12 : HH);
	...
	public static String ticksToTimeString (long ticks)
	{
		...
		if (days > 0L) time = days + ":";

		if (hrs > 0L) time = time + ( ( (hrs % 24L) < 10L) && (days > 0L) ? "0" : "") + (hrs % 24L) + ":";

		if (mins > 0L) time = time + ( ( (mins % 60L) < 10L) && (hrs > 0L) ? "0" : "") + (mins % 60L) + ":";
		else time = time + "0:";
	...
	public static String[] loadTextFromURL (URL url, Logger logger, String[] defaultValue, int timeoutMS)
	{
		...
		catch (Throwable e)
		{
			...
			if (bspkrsCoreMod.instance.allowDebugOutput) e.printStackTrace();
	...
	public static RayTraceResult getPlayerLookingSpot (EntityPlayer player, boolean restrict)
	{
		...
		{
			// couldn't find player reach in 1.10.2
			distance = 4;
		}
